...
def fetch_contest_data() -> dict[str, PlatformResponse]:
    ...
    try:
        lc = requests.get(
            "https://alfa-leetcode-api.onrender.com/contests/upcoming", timeout=10
        )
        lc.raise_for_status()
        ...
        all_data["leetcode"] = PlatformResponse(
            success=False, error=f"Network error: {str(e)}"
        )
    except (KeyError, ValueError) as e:
        all_data["leetcode"] = PlatformResponse(
            success=False, error=f"Data parsing error: {str(e)}"
        )
    ...
    try:
        ...
        all_data["codeforces"] = PlatformResponse(
            success=False, error=f"Network error: {str(e)}"
        )
    except (KeyError, ValueError) as e:
        all_data["codeforces"] = PlatformResponse(
    ...
    try:
        ...
        all_data["codechef"] = PlatformResponse(
            success=False, error=f"Network error: {str(e)}"
        )
    except (KeyError, ValueError) as e:
        all_data["codechef"] = PlatformResponse(
